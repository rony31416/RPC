...
1. **Install Node.js**
   Ensure Node.js is installed on the Ubuntu machine. If not, install it using:
   ```bash
   sudo apt update
...
4. **Find the IP Address of Machine 1**
   Run the following command to find the machine's IP address:
   ```bash
   ip a
   ```
